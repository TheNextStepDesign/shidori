"use client"
import { Base, H3, Paragraph } from "@/components/typography/Typography";
...
export default function FeaturedCard({data}) {
    ...
    const {button,singleData,gallary,name} = data
  ...
  return (
        ...
        <div className="bg-white rounded-xl flex flex-col md:flex-row md:rounded-3xl " >
            ...
            <FeaturedImgCarousel gallary={gallary} setIndex={setIndex} name={name} />
            <div className="w-full md:w-1/2 flex flex-row justify-center items-center relative ">
                ...
                <p  className={'text-paragraph absolute top-10 right-10 bg-accent p-2 rounded-xl font-semibold hidden md:block'} >{name}</p>
            </div>
        </div>
